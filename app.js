...
(function () {
  ...
  // Nota de pie con los conceptos que cubre la instalación (se arma una
  // sola vez a partir de data.js, para no repetir la lista a mano).
  const INSTALACION_NOTA = `Incluye instalación de: ${INSTALACION.articulos.join(", ")}. ${INSTALACION.nota}`;
  ...
  function selectArticulo(entry) {
    ...
    // Instalación solo se ofrece para GE de Línea Blanca y Electrónica y Cómputo
    stateA.aplicaInstalacion = !!entry.categoria.aplicaInstalacion;
  ...
  function mostrarResultado({ detalle, opcion, precio, instalacionOn }) {
    ...
    if (instalacionOn) {
      ...
      installNota.textContent = INSTALACION_NOTA;
